...
def display_station_info(stations, query_lat, query_lon):
    for station in stations:
        ...
        distance_str = (
            f"{haversine_distance(query_lat, query_lon, station_lat, station_lon):.2f} km"
        ...
        print(
            Fore.CYAN
            + Style.BRIGHT
            + f"Station Name: {station.get('name', 'N/A')}{emoji}"
        )
        addr = station.get("address", {})
        address_text = f"{addr.get('streetAndHouseNumber', 'N/A')}, {addr.get('postcode', '')} {addr.get('city', '')}"
        clickable_address = (
            format_google_maps_link(address_text, station_lat, station_lon)
            if station_lat and station_lon
            else address_text
        )
        print(Style.BRIGHT + f"Address: {clickable_address}")
        print(
            f"Coordinates: lat={station_lat}, lng={station_lon} (Distance: {distance_str})"
        )

        access_type = station.get("accessType", "N/A")
        access_color = Fore.GREEN if access_type.lower() == "public" else Fore.YELLOW
        print("Access Type: " + access_color + access_type + Style.RESET_ALL)

        owner = station.get("owner") or {}
        owner_name = owner.get("name", station.get("ownerName"))
        if owner_name:
            owner_website = owner.get("website", None)
            if owner_website:
                clickable_owner = (
                    f"\033]8;;{owner_website}\033\\{owner_name}\033]8;;\033\\"
                )
            else:
                clickable_owner = owner_name
            print("Owner: " + Fore.MAGENTA + clickable_owner + Style.RESET_ALL)
        # else: do not print owner info if missing.

        evses = station.get("evses", [])
        total = len(evses)
        occupied = sum(
            1 for evse in evses if evse.get("status", "").upper() != "AVAILABLE"
        )
        if total > 0:
            usage_color = (
                Fore.RED
                if occupied == total
                else Fore.GREEN if occupied == 0 else Fore.YELLOW
            )
            print(
                Style.BRIGHT
                + f"Usage: {usage_color}{occupied}/{total}{Style.RESET_ALL} sockets"
            )
        ...
        for evse in evses:
            ...
            relative_time = (
                f"({get_relative_time(last_updated_str)})" if last_updated_str else ""
            )
            ...
            print(
                f"    Status: {status_color}{status} {relative_time} {free_icon}{Style.RESET_ALL}"
            )
            print("    Connectors:")
            for connector in evse.get("connectors", []):
                ...
                if max_power != "N/A":
                    ...
                    max_power_str = max_power
                print(
                    f"       * ID: {connector.get('id', 'N/A')}, Standard: {connector.get('standard', 'N/A')}, "
...
def main():
    ...
    group.add_argument(
        "--address",
        type=str,
        help="Address to search around (e.g., '1600 Amphitheatre Parkway, Mountain View, CA').",
    )
    group.add_argument(
        "--coordinate",
        nargs=2,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="Range in kilometers around the center (default: 0.1 km).",
    )
    parser.add_argument(
        "--min-speed",
        type=int,
        default=1,
        help="Minimum charging speed in kW (default: 1).",
    )
    parser.add_argument(
        "--max-speed",
        type=int,
        default=999,
        help="Maximum charging speed in kW (default: 999).",
    )
    parser.add_argument(
        "--available-now",
        action="store_true",
    ...
    stations = get_charging_stations(
        bounds, available_now=args.available_now, speed_range=speed_range
    )
